...
3. `# ─── Functions ───...`
4. `# ─── Main ───...` (directly-executed scripts only; **omit for sourced lib files**)
5. `# ─── Constants / globals ───...`
6. `# ─── Imports ───...` (lib files that re-source dependencies)
7. `# ─── Execute ───...` (directly-executed scripts only): a single line that
